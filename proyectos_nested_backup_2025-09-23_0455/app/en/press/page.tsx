...
export const metadata = { title: 'Press — Sierra' }
    ...
    <main className="section">
      <div className="container">
        ...
        <div className="card" style={{marginTop:12}}>
          <h3 style={{marginTop:0}}>PRESS MATERIALS</h3>
          {files.length === 0 ? (
            <p className="small">Upload files to <code>/public/assets/press</code>.</p>
          ) : (
            <ul className="list" style={{marginTop:8}}>
              {files.map((f, i) => (
                <li key={i} style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center'}}>
                  <span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{f.name}</span>
                  <a className="btn" href={f.href} download>Download</a>
                </li>
              ))}
            </ul>
          )}
